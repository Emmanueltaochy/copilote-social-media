...
import { absolutePath, isBrandingKind } from "@/lib/storage";
 ...
 * relu dans les réglages. Trois images, pas une de plus.
 */
export async function GET(_request: Request, { params }: { params: Promise<{ kind: string }> }) {
  const { kind } = await params;
  if (!isBrandingKind(kind)) return new Response("Introuvable", { status: 404 });
